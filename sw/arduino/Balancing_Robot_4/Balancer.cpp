...
Balancer::Balancer(const int zeroAngleOffset)
: Balancer(0.0, 0.0, 0.0, zeroAngleOffset)
{
	// Nothing to do
}
...
void Balancer::setKP(float newKP) {
	pid.setKp(newKP);
	resetSpeed();
}

void Balancer::setKI(float newKI) {
	pid.setKi(newKI);
	resetSpeed();
}

void Balancer::setKD(float newKD) {
	pid.setKd(newKD);
	resetSpeed();
}
